// /entry-engine
// ├── entrySignalsRenderer.js   // головний engine + логіка сигналів
// ├── derivedMetrics.js       // derived metrics (trend, impulse, volume, volatility...)
// ├── ctEngine.js             // counter-trend engine (CT логіка)
// └── marketUtils.js   
...
export function renderEntrySignals({
  ...
  if (!activeEntrySignals || activeEntrySignals.length === 0) {
    el.innerHTML =
      `<div class="no-signals">No valid entry conditions detected (умови для входу відсутні).</div>`;
    return;
  }
  ...
  activeEntrySignals.forEach((sig) => {
    ...
    // CONTEXT REASONS
    if (sig.context === "trend_add") reasons.push("Trend-following continuation setup");
    if (sig.context === "reversal") reasons.push("Mean-reversion / reversal setup");
    if (sig.context === "htf_add") reasons.push("Higher timeframe confirmation");
    if (sig.context === "range") reasons.push("Range-based setup");
...
out += `</div>`; // close entry-block

document.getElementById("outputFuaters").innerHTML = out;
...
document.getElementById("outputCt").innerHTML =
  ctHTML && ctHTML.trim().length > 0
    ? ctHTML
    : `<div>No counter-trend conditions detected.</div>`;
  });
}